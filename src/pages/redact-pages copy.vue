<template>
  <div>
    ...
    <!-- 通过v-if判断传入的attribute的类型，进行不同的展示 -->
    <!-- 单节课的编辑页面 -->
        <!-- 单节课页面 -->
    <!-- 外部大盒子 -->
    <div class="single" v-if="$route.query.attribute == '单节课'">
      <!-- 内部盒子 -->
      <div class="banner-spared">
        ...
        >
          ...
          <div class="el-upload__text">拖动视频资源到此处，或者选择视频资源<em>上传</em></div>
        </el-upload>
        </div>
      ...
      <div class="img-banner">
        ...
        >
          <i class="el-icon-upload"></i>
          <div class="el-upload__text">拖动图片到此处，或者选择图片<em>上传</em></div>
        </el-upload>
      </div>
      <!-- 下方文字描述区域 -->
      ...
      <div class="select-class select-public">
        <span class="point-public"></span>
        类目选择
        <el-select v-model="this.$route.query.cate" placeholder="请选择" class="el-select">
          <el-option
...
methods:{
  ...
  getSingleList(){
    this.$http.post('/course/edit',{
      ...
      this.course_cover_url = res.data.data.course.course_cover_url
      this.cate = res.data.data.course.cate
      this.posid = res.data.data.course.posid
    })
  }
}
}
