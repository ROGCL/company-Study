...
import { Behavior_Statistics_startTime } from "@/config/api/index";
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableData: (state) => state.start_times.list,
    }),
    timeTypeConfig() {
  ...
  methods: {
    ...mapActions(["getStartTimesList", "getStartTimesCount"]),
    onSubmit(res) {
    ...
    async fetchList() {
      ...
      const [err, res] = await this.getStartTimesList({
        ...searchForm,
        ...tableOpt,
        navCate: 2,
