<template>
  <div class="home">
    ...
    >
      <template #title>
        <div class="count_title">日期活跃趋势</div>
      </template>
      <template #cetent>
        <el-radio-group
          v-model="allOrinfo"
    ...
    >
      <template #title>
        <div class="count_title">版本活跃趋势</div>
      </template>
      <template #cetent>
        <el-radio-group
          v-model="vAllOrinfo"
    ...
    >
      <template #device_active_vip_per="row">{{
        row.row.device_active_vip_per | formatPercentage
      }}</template>
    </TableLayout>
  </div>
</template>
...
import { User_Statistics_active } from "@/config/api/index";
...
export default {
  ...
  data() {
    return {
      ...
      column: [
        ...
        {
          prop: "device_active_vip",
          label: "活跃会员（设备）",
        },
        {
          prop: "device_active_vip_per",
          label: "活跃会员占比",
        },
      ],
      tableOpt: {
  ...
  computed: {
    ...mapState({
      ...
      tableData: (state) => state.active_user.list,
    }),
    timeTypeConfig() {
  ...
  methods: {
    ...mapActions(["getActiveUserList", "getActiveUserCount"]),
    onSubmit(res) {
    ...
    async fetchList() {
      ...
      const [err, res] = await this.getActiveUserList({
        ...searchForm,
        ...tableOpt,
        navCate: 2,
    ...
    async versionData() {
      ...
      const [err, res] = await this.$http.post("User_Statistics/active", {
        ...searchForm,
        channel: searchForm["channel"].join(","),
        version: searchForm["version"].join(","),
        navCate: 6,
