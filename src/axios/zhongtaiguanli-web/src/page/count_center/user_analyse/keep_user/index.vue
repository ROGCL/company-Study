...
import { User_Statistics_keep } from "@/config/api/index";
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableData: (state) => {
        arr = [];
        let obj = state.keep_user.list.data;
        ...
        const filterData = state.keep_user.list.data.map((item) => {
          const obj = {};
        ...
        for (const key in state.keep_user.list.data) {
          state.keep_user.list.data[key] = filterData[key];
        }

        return state.keep_user.list;
      },
      count: (state) => state.keep_user.count.data,
      countLoading: (state) => state.keep_user.count.loading,
    }),
    countData() {
      const { count } = this;
      ...
      if (this.allOrinfo == 1 && this.searchForm.channel.length > 0) {
        ...
        console.log(xAxisData, legendData, data);
        return {
          legendData,
          xAxisData,
          data,
  ...
  methods: {
    ...mapActions(["getKeepUserList", "getKeepUserCount"]),
    onSubmit(res) {
    ...
    async fetchList() {
      ...
      const [err, res] = await this.getKeepUserList({
        ...searchForm,
        ...tableOpt,
        navCate: 2,
    ...
    async fetchCount() {
      ...
      const [err, res] = await this.getKeepUserCount({
        ...searchForm,
        select,
        navCate: 1,
        channel: searchForm["channel"].join(","),
