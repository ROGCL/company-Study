...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableData: (state) => state.order_income.list,
      count: (state) => state.order_income.count.data,
      countLoading: (state) => state.order_income.count.loading,
    }),
    countData() {
      const { count } = this
  ...
  methods: {
    ...mapActions(['getOrderIncomeList', 'getOrderIncomeCount']),
    onSubmit(res) {
    ...
    async fetchList() {
      ...
      const [err, res] = await this.getOrderIncomeList({
        ...searchForm,
        ...tableOpt,
        navCate: 2,
