...
export default {
  ...
  methods: {
    ...
    async getParamDetail() {
      const [err, res] = await this.$http.post("event/customFieldList");
      if (err) return;
    ...
    async getReport() {
      ...
      const [err, res] = await this.$http.post("event/predict", {
        ...searchForm,
        ...tableOpt,
        ...sort,
        list_group: group.sort(function (a, b) {
      ...
      for (const key in sum) {
        if (this.numKeyArr.indexOf(key) !== -1) {
          sum[key] = parseFloat(sum[key]).toLocaleString();
        }
        if (this.doubleDigitArr.indexOf(key) !== -1) {
