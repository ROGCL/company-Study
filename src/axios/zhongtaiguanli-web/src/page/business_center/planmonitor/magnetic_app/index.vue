  ...
  <div class="home">
    ...
    >
      <template #table-option>
        <el-button
          type="primary"
          @click="$router.push('/business_center/planmonitor/magnetic_app/authorize')"
        >授权管理</el-button>
      </template>
      ...
      <template #option="scope">
        <el-popconfirm
          ...
          :title="scope.row.status == 1 ? '是否确认开启该账户？（账户开启后，将拉取账户数据）': '是否确认关闭该账户？（账户关闭后，将停止拉取账户数据）'"
        >
          <el-switch slot="reference" :value="scope.row.status == 1 ? true : false " inactive-color="#ff4949"></el-switch>
        </el-popconfirm>
        <span
          class="mg-left"
