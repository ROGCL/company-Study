    ...
    <FilterLayout v-model="searchForm" @onSubmit="onSubmit">
      <el-col :md="6" :sm="12">
        <el-form-item label="广告主ID" prop="account_id">
          <el-input v-model="searchForm.account_id" placeholder="请输入广告主ID" />
        </el-form-item>
      </el-col>
      <el-col :md="6" :sm="12">
    ...
    >
      ...
      <template #option="scope">
        <el-popconfirm
          ...
          :title="scope.row.status == 1 ? '是否确认关闭该账户？（账户关闭后，将停止拉取账户数据）' : '是否确认开启该账户？（账户开启后，将拉取账户数据）'"
        >
          <el-switch
            slot="reference"
        ...
        <span
          class="mg-left"
          :style="scope.row.status == 1 ? ' color: #1890ff;' : ' color: #ff4949;'"
        >{{scope.row.status == 1 ? '开启' : '关闭' }}</span>
      </template>
    </TableLayout>
    <el-dialog title="修改权限token" :visible.sync="dialogFormVisible">
      <el-form :model="tokenForm">
        <el-form-item label="权限token：" :label-width="formLabelWidth" prop="token">
          <el-input v-model="tokenForm.token"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="doneDialog">取 消</el-button>
        <el-button @click="showDialog">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  // 获取筛选项表单数据
    ...
    async showDialog(){
      const [err,res] = await this.$http.post('Invest_Config/editBdToken',{
        id:this.tokenForm.id,
